...
bool CMainFrame::ThreadProc1()
{
	...
	CString url = L"http://211.157.104.87:8080/sipo/zljs/hyjs-jieguo.jsp?recshu=20&searchword="+(CString)searchinfo.c_str()+L"&flag3=1&pg=1&sign=0";/*L"&flag3=1&pg=1&sign=0"urltail*/
...
void CMainFrame::ParsePatentDetails(string pgcontent ,string save_as_path,CString application_number)
{
	...
	Patent_Specification_Download((string)CN_temp_app_num,patent_specification_url,patent_specification_total_page,save_as_path,&lvitem);
	/****************************************************************************************/
}

void CMainFrame::Patent_Specification_Download(string patent_num,string specification_url, string pgtotal,string save_as_path,LVITEM *lvitem)
{
...
bool CMainFrame::ThreadProc5() // 开始下载未下载完成的
{
	...
	for(i=nItemCount-1;i>=0;i--)
	{
		...
		if(iState!=0)
		{
			...
			CString sqlsentence;
			sqlsentence.Format( L"select web_url from search_history_item where application_number='%s'",cstr_Number);
			if (m_runsql.CheckSQLResult(sqlsentence))		//存在数据
			{
				cstr_URL = (LPCTSTR)(_bstr_t)m_runsql.m_recordset->GetCollect("web_url");
